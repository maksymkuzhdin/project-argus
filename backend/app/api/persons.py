...
def _snap_to_dict(s: YearlySnapshot) -> dict[str, Any]:
    return {
        ...
        "total_real_estate": str(s.total_real_estate) if s.total_real_estate is not None else None,
        "total_assets": str(s.total_assets) if s.total_assets is not None else None,
...
def _change_to_dict(c: YOYChange) -> dict[str, Any]:
    ...
    return {
        ...
        "assets_prev": _fmt(c.assets_prev),
        "assets_curr": _fmt(c.assets_curr),
        "asset_growth": c.asset_growth,
        "cash_prev": _fmt(c.cash_prev),
        "cash_curr": _fmt(c.cash_curr),
        "cash_delta": _fmt(c.cash_delta),
        "unknown_share_prev": c.unknown_share_prev,
        "unknown_share_curr": c.unknown_share_curr,
        "unknown_share_delta": c.unknown_share_delta,
        "role_prev": c.role_prev,
        "role_curr": c.role_curr,
        "role_changed": c.role_changed,
        "major_assets_appeared": c.major_assets_appeared,
        "major_assets_disappeared": c.major_assets_disappeared,
        "max_appeared_value": _fmt(c.max_appeared_value),
        "max_disappeared_value": _fmt(c.max_disappeared_value),
        "one_off_income_curr": _fmt(c.one_off_income_curr),
    }
